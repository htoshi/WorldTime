...
/* zeller の公式により曜日を求める
	...
	戻り値: 0 日曜日～6 土曜日
*/
int CMainDlg::zeller(int iYear, int iMonth, int iDay)
{
	...
	return ((5*iYear/4 - iYear/100 + iYear/400 + (26*iMonth + 16)/10 + iDay) % 7);
}
...
/* 月の日数を求める
	...
	戻り値: 月の日数
*/
int CMainDlg::getDayOfMonth(int iYear, int iMonth)
{
...
*/
int CMainDlg::isDateAfterSpecifiedDate(int iBaseMonth, int iBaseDay, int iMonth, int iDay)
{
	// 基準日よりも後
...
/* 月の最初のN曜日の日付を求める
	...
	戻り値: 月の最初のN曜日の日付
*/
int CMainDlg::getMonthFirstWeekDay(int iYear, int iMonth, int iWeekDay)
{
	// 1日の曜日を求める
...
/* 月の最後のN曜日の日付を求める
	...
	戻り値: 月の最後のN曜日の日付
*/
int CMainDlg::getMonthLastWeekDay(int iYear, int iMonth, int iWeekDay)
{
	// 月の最後の日を求める
...
*/
int CMainDlg::getMonthMWeekDay(int iYear, int iMonth, int iNum, int iWeekDay)
{
	// 月の最初のN曜日の日付を求める
...
*/
bool CMainDlg::isDataInSpecifiedRange(int iMonth, int iDay,
	int iFromMonth, int iFromDay, int iToMonth, int iToDay, bool isInclude)
{
	// 指定した月の範囲外
...
/* datetime_t を与えて任意の時差を与えた時の日付と時刻を得る
	...
	戻り値: OK
*/
int CMainDlg::getDateTimeByOffset(datetime_t* dt, int iOffsetMin)
{
	int iHour, iMin;
...
/* 1日の中で累積分を求める
	...
	戻り値: 1日の中での累積分
*/
int CMainDlg::getTotalMinutesInDay(int iHour, int iMin)
{
...
/* 現在のGMT時刻を取得 (グリニッジ標準時)
	...
	戻り値: OK
*/
int CMainDlg::getCurrentGMT(datetime_t* dt)
{
	SYSTEMTIME stTime;
...
/* DST 日付を設定
	...
	戻り値: OK
*/
int CMainDlg::setDSTDay(dstinfo_t* dstInfo, int iYear)
{
	// 月の第一週の曜日の日付を設定
	if(dstInfo->Type == 'F'){
...
/* 世界の任意時差・指定されたサマータイムで時刻を取得
	...
	戻り値: OK
*/
int CMainDlg::getWorldTime(datetime_t* dt, datetime_t* dt_in, double offset,
